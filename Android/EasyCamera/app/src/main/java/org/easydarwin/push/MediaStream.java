...
public class MediaStream {
    ...
    private boolean mRequestSnapshot;
...
//    }
    ...
    private void initPusher(String ip, String port, String id, EasyPusher.OnInitPusherCallback callback) {
        try {
            mEasyPusher.initPush(ip, port, id, mApplicationContext, callback);
        } catch (Exception e) {
            Log.i("", "");
        }
    }
...
//                ev.setFramerate(25);
    ...
    private Camera.CameraInfo camInfo = new Camera.CameraInfo();
    Camera.PreviewCallback previewCallback = new Camera.PreviewCallback() {
        ...
        public void onPreviewFrame(byte[] data, Camera camera) {
            ...
            if (mRequestSnapshot){
                mRequestSnapshot = false;
                final String path = new File(mApplicationContext.getExternalFilesDir(Environment.DIRECTORY_PICTURES), "snap.jpeg").getPath();
                try {
                    if (mDgree == 0) {
                        int cameraRotationOffset = camInfo.orientation;

                        byte[]buffer = new byte[data.length];
                        int y_stride = previewSize.width * previewSize.height;
                        if (cameraRotationOffset == 90) {
                            if (mSWCodec) {
                                System.arraycopy(data, 0, buffer, 0, y_stride);
                                for (int i =0;i< y_stride /4;){
                                    buffer[y_stride + i*2] = data[y_stride + i];
                                    buffer[y_stride + i*2+1] = data[y_stride +y_stride/4 + i];
                                    ++i;
                                }
                                yuvRotate(buffer, 1, previewSize.width, previewSize.height, 90);
                            }else{
                                System.arraycopy(data, 0, buffer, 0, data.length);
                                yuvRotate(buffer, 1, previewSize.width, previewSize.height, 90);
                            }
                            yuvToJpeg(buffer, previewSize.height, previewSize.width, 50, path);
                        } else if (cameraRotationOffset == 270) {
                            if (mSWCodec) {                                System.arraycopy(data, 0, buffer, 0, y_stride);
                                for (int i =0;i< y_stride /4;){
                                    buffer[y_stride + i*2] = data[y_stride + i];
                                    buffer[y_stride + i*2+1] = data[y_stride +y_stride/4 + i];
                                    ++i;
                                }
                                yuvRotate(buffer, 1, previewSize.width, previewSize.height, 270);
                            }else{
                                System.arraycopy(data, 0, buffer, 0, data.length);
                                yuvRotate(buffer, 1, previewSize.width, previewSize.height, 270);
                            }
                            ...
                            yuvToJpeg(buffer, previewSize.width, previewSize.height, 50, path);
                        }
                    } else {
                        yuvToJpeg(data, previewSize.width, previewSize.height, 50, path);
                    }
                    LocalBroadcastManager.getInstance(mApplicationContext).sendBroadcast(new Intent(ACTION_SNAPSHOT_GOT));
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
